...
std::unique_ptr<ILoader> CreateWAVLoader(fs::path const &path) {
  return std::make_unique<WAVLoader>(path);
}
...
Data WAVLoader::loadData() {
  ...
  std::size_t channels = static_cast<std::size_t>(m_audioFile.getNumChannels());
  std::size_t samples =
  ...
  if (freq == 0 || channels == 0 || samples == 0 || depth == 0) {
    ...
    errStream << "Invalid WAV file data: sample-rate = " << freq
              << ", channels = " << channels
              << ", samples-per-channel = " << samples << ", depth = " << depth;
...
std::unique_ptr<ISaver> CreateWAVSaver(fs::path const &path) {
  return std::make_unique<WAVSaver>(path);
}
...
void WAVSaver::saveData(Data const &data) {
  if (data.freq == 0 || data.channels == 0 || data.depth == 0) {
    std::stringstream errStream;
    errStream << "Invalid WAV file sample-rate = " << data.freq
              << " or channels = " << data.channels
              << " or depth = " << data.depth;
